...
export class PortComponent {
  ...
  ngOnInit(): void {
    ...
    this.portSearchForm = this.fb.group({
      port_name: [''],
      port_code: [''],
      region_id: [''],
      country_id: [''],
  ...
  loadData() {
    ...
    this.api.get('get-port-range').subscribe({
      next: (response: any) => {
        this.portrangeList = response.data;
      },
      error: (error: any) => {},
      complete: () => {},
    });
  ...
  initializeData() {
    this.api.getWithPaginate('get-port').subscribe({
      next: (response: any) => {
  ...
  onPerPageChange(event: any) {
    ...
    this.api.getWithPerPage('get-port', '', allParams, true).subscribe({
      next: (response: any) => {
        this.portList = response.data.data;
        this.pagination = response.data.links;
        this.fromStart = {
  ...
  editSelect(id: number) {
    this.api.get('edit-get-port/' + id).subscribe({
      next: (response: any) => {
  ...
  onSubmit() {
    if (this.portForm.valid) {
      this.showLoader();
      this.api.post('port', this.portForm.value).subscribe({
        next: (response: any) => {
  ...
  onUpdate() {
    if (this.portForm.valid) {
      this.showLoader();
      this.api.update('port', this.portForm.value).subscribe({
        next: (response: any) => {
          if (response.status === true) {
            this.toasterService.success(
  ...
  deleteData(id: number, name: string): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.showLoader();
        this.api.delete('delete-port/' + id).subscribe({
          next: (response: any) => {
            this.toasterService.success(
  ...
  onSearch() {
    if (this.portSearchForm.valid) {
      this.showLoader();
      this.api.getSearch('get-port', this.portSearchForm.value).subscribe({
        next: (response: any) => {
  ...
  resetSearchForm() {
    this.portSearchForm.patchValue({
      port_name: '',
      port_code: '',
      country_id: '',
      port_range: '',
